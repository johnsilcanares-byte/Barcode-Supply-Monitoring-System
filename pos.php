...
// --- ADDED: Ensure scanner_approved column exists (run once) ---
try {
    $db->exec("ALTER TABLE users ADD COLUMN IF NOT EXISTS scanner_approved TINYINT(1) NOT NULL DEFAULT 0");
} catch (PDOException $e) {
    // Column might already exist – ignore
}

// --- ADDED: Fetch user's scanner approval status ---
$stmt = $db->prepare("SELECT scanner_approved FROM users WHERE id = ?");
$stmt->execute([$_SESSION['user_id']]);
$userData = $stmt->fetch(PDO::FETCH_ASSOC);
$scannerApproved = ($userData && $userData['scanner_approved'] == 1) || $_SESSION['role'] === 'admin';

// --- ADDED: Check if user already has a pending request ---
$hasPendingRequest = false;
if (!$scannerApproved && $_SESSION['role'] !== 'admin') {
...
if (isset($_POST['barcode']) || isset($_POST['camera_barcode'])) {
    $barcode = trim($_POST['barcode'] ?? $_POST['camera_barcode'] ?? '');
    $scannedBarcode = $barcode;
    // Trim both sides and optionally use case‑insensitive collation
    $stmt = $db->prepare("SELECT id, name, stock, price, barcode FROM inventory WHERE TRIM(barcode) = TRIM(?)");
    $stmt->execute([$barcode]);
    $itemDetails = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$itemDetails && !empty($barcode)) {
        // One more try: remove any non‑alphanumeric characters (common with some scanners)
        $cleanBarcode = preg_replace('/[^A-Za-z0-9]/', '', $barcode);
        if ($cleanBarcode !== $barcode) {
            $stmt = $db->prepare("SELECT id, name, stock, price, barcode FROM inventory WHERE TRIM(barcode) = TRIM(?)");
            $stmt->execute([$cleanBarcode]);
            $itemDetails = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($itemDetails) $scannedBarcode = $cleanBarcode;
        }
    }
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .scanner-toggle, .btn-switch-camera {
        ...
        .scanner-toggle:hover, .btn-switch-camera:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(22,163,74,0.4); }
        ...
        @media (min-width: 1201px) { .menu-toggle { display: none; } }
        ...
        @media (max-width: 768px) {
            ...
            .sidebar-logo { width: 90px; height: 90px; }
            .sidebar-brand h4 { font-size: 0.9rem; }
        }
        ...
        <div class="content-wrapper">
            ...
            <div class="app-main">
                <div class="left-panel">
                    <!-- Scanner Section with optional lock overlay -->
                    <div class="scanner-card" id="scannerCard">
                        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <button id="scanner-toggle" class="scanner-toggle" <?= !$scannerApproved ? 'disabled style="opacity:0.5;"' : '' ?>><i class="fas fa-play me-1"></i>Start</button>
                        ...
                        <!-- LOCK OVERLAY (only shown if scanner not approved and not admin) -->
                        <?php if (!$scannerApproved && $_SESSION['role'] !== 'admin'): ?>
                        <div class="scanner-lock-overlay" id="scannerLockOverlay">
                            <i class="bi bi-lock-fill"></i>
                            <h4>Scanner Locked</h4>
                            <p>Your barcode scanner access is pending admin approval.</p>
                            <?php if ($hasPendingRequest): ?>
                                <div class="alert alert-warning mt-2 py-2 px-3 rounded-pill" style="background: rgba(255,193,7,0.2); border: none;">
                                    <i class="bi bi-clock-history"></i> Request already sent. Please wait for admin approval.
                                </div>
                            <?php else: ?>
                                <button class="btn-request-access" id="requestAccessBtn">
                                    <i class="bi bi-envelope-paper"></i> Send Request to Admin
                                </button>
                            <?php endif; ?>
                        </div>
                        <?php endif; ?>
                    ...
                    <!-- Manual Barcode Entry (always accessible) -->
...
<!-- MODAL: Request Sent Confirmation -->
<div class="modal fade" id="requestModal" tabindex="-1" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 rounded-4">
            <div class="modal-body text-center p-4">
                <i class="bi bi-send-check-fill" style="font-size: 4rem; color: #2e9c5e;"></i>
                <h4 class="mt-3">Request Sent!</h4>
                <p>Your approval request has been sent to the administrator. You will be able to use the barcode scanner once approved.</p>
...
// ----- SCANNER LOGIC (only runs if scannerApproved = true) -----
<?php if ($scannerApproved || $_SESSION['role'] === 'admin'): ?>
...
const MIN_SCAN_CONFIDENCE = 0.4;   // more tolerant
...
function initializeQuagga() {
    Quagga.init({
        inputStream: {
            ...
            // Focus on the center area where barcode usually appears
            area: {
                top: "20%",
                right: "10%",
                left: "10%",
        ...
        locator: {
            patchSize: "large",       // better for small/distant barcodes
        ...
        locate: true,
        frequency: 15                 // more scans per second
    }, (err) => {
        if (err) {
            console.error("Quagga init error:", err);
            setScannerStatus('Error', 'error');
            updateScanFeedback('Scanner failed – try manual entry');
            return;
        }
        Quagga.start();
    ...
    Quagga.onDetected(result => handleBarcodeDetection(result));
    // No need for onProcessed – onDetected is enough
}
...
<?php else: ?>
// Scanner is locked – disable buttons and show lock overlay
document.getElementById('scanner-toggle')?.setAttribute('disabled', 'disabled');
document.getElementById('switch-camera')?.setAttribute('disabled', 'disabled');
<?php endif; ?>

// ----- REQUEST ACCESS BUTTON (only if lock overlay exists) -----
const requestBtn = document.getElementById('requestAccessBtn');
if (requestBtn) {
    requestBtn.addEventListener('click', function() {
        fetch('request_scanner_access.php', {
            method: 'POST',
        ...
        .then(data => {
            if (data.success) {
                ...
                // Optionally refresh the page after modal close to show pending status
                ...
                Swal.fire({
                    ...
                    text: data.message || 'Could not send request. Please try again later.',
                    confirmButtonColor: '#16a34a'
                });
            }
        })
